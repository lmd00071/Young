...
//        
//    }
    self.userNameLabel.text=userMessage.user_account_uid;
...
- (void)leftAction{
    ...
    [SVProgressHUD dismiss];
     self.Getblock(self.goldNumberLabel.text);
...
{
    if (indexPath.section == 1 && indexPath.row == 0) {     //兑奖区
        BWRewardAreaController *rewardAreaVc = [[BWRewardAreaController alloc] init];
        rewardAreaVc.goldNumber = self.goldNumberLabel.text;
        [self.navigationController pushViewController:rewardAreaVc animated:YES];
    }
    if (indexPath.section == 1 && indexPath.row == 1) {     //兑奖历史
        BWRewardHistoryController *rewardHistoryVc = [[BWRewardHistoryController alloc] init];
        [self.navigationController pushViewController:rewardHistoryVc animated:YES];
    }
    if (indexPath.section == 2 && indexPath.row == 0) {     //收益历史
...
- (void)setupHttpRequest
{
    ...
    parameter[@"trans_code"] = @"user_account_info_get";
